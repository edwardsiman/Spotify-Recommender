...
from call_function import offset_api_limit, get_artists_df, get_tracks_df, get_track_audio_df,\
    get_all_playlist_tracks_df, get_recommendations


with open("spotify/spotify.yml", 'r') as stream:
...
top_artists = offset_api_limit(sp, sp.current_user_top_artists(limit=20, offset=0, time_range='medium_term'))
...
followed_artists = offset_api_limit(sp, sp.current_user_followed_artists(limit=20))
...
top_tracks = offset_api_limit(sp, sp.current_user_top_tracks(limit=20, offset=0, time_range='medium_term'))
...
saved_tracks = offset_api_limit(sp, sp.current_user_saved_tracks(limit=20, offset=0))
...
playlist_tracks_df = get_all_playlist_tracks_df(sp, sp.current_user_playlists(limit=40, offset=0))  # limit of 50 playlists by default
